...
const ThuongHieu = () => {
    ...
    const showData=(page)=>{
        brandService.getAllPaging(page).then(res=>{
    ...
    const getPage=async ()=>{
        const myPage=[];
        await brandService.getTotalPage().then(res=>{
        ...
        if(validate(name)===0){
            if(whatAction==='add'){
                ...
                setTimeout(async ()=>{
                    ...
                    await brandService.update(id,brandRequest).catch(e=>{
                        console.log(e)
                    })
                    await showData(currentPage);
                    await setLoading(false)
                    await value.showToastMessage("Cập Nhật Thương Hiệu Thành Công!")
                    await handleClose();
                },1000)
            }
        }
    ...
    const handleEdit=(id)=>{
        setWhatAction('edit')
        setId(id)
        brandService.getById(id).then(res=>{
    ...
    const movePage=(page)=>{
        if(whatActionMovePage==='find'){
            brandService.findByAll(nameFind,statusFind,page).then(res=>{
                setList(res.data)
            }).catch(e=>{
                console.log(e)
            })
    ...
    const handleMovePre=()=>{
        if(whatActionMovePage==='find'){
            if(currentPage===1){
                brandService.findByAll(nameFind,statusFind,totalPage.length).then(res=>{
                    setList(res.data)
                }).catch(e=>{
                    console.log(e)
                })
                ...
                brandService.findByAll(nameFind,statusFind,currentPage-1).then(res=>{
                    setList(res.data)
                }).catch(e=>{
                    console.log(e)
                })
    ...
    const handleMoveNext=()=>{
        if(whatActionMovePage==='find'){
            if(currentPage===totalPage.length){
                brandService.findByAll(nameFind,statusFind,1).then(res=>{
                    setList(res.data)
                }).catch(e=>{
                    console.log(e)
                })
    ...
    const handleFind= async ()=>{
        ...
        await brandService.findByAll(nameFind,status,1).then(res=>{
            setList(res.data)
        }).catch(e=>{
            console.log(e)
        })
        await brandService.findByAllTotalPage(nameFind,statusFind).then(res=>{
            let myArray=[]
            for(let i=1;i<=res.data;i++){
                myArray.push(i);
            }
            ...
            <div className="">
                <div className="p-[20px]">
                    <div className="title mb-[20px]">
                        ...
                        <span className="text-[22px] font-bold ml-[10px]">Quản lý thương hiệu</span>
                    </div>
                            ...
                            <div className="py-[20px]">
                                <div className="grid grid-cols-2">
                                    <div className="text-center">
                                        <span className="text-[#444] text-[14px] font-[500]">Tên thương hiệu :</span>
                ...
                <div className="flex justify-center items-center h-full">
                    <div className="modal-container p-[20px] bg-[#fff] rounded-[5px]">
                        <div className="flex justify-between">
                            <div><span className="font-[600]">Thêm thương hiệu</span></div>
                            <div className="cursor-pointer" onClick={handleClose}><i className="fa-solid fa-xmark"></i></div>
                        </div>
                        <div className="text-[14px]">
                            <div className="flex items-center">
                                <p className="mb-[5px] mt-[10px]"><span className="text-primary-red">*</span> Tên thương hiệu</p>
