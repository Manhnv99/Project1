...
const TheLoai = () => {
    ...
    const showData=(page)=>{
        categoryService.getAllPaging(page).then(res=>{
    ...
    const getPage=async ()=>{
        const myPage=[];
        await categoryService.getTotalPage().then(res=>{
        ...
        if(validate(name)===0){
            if(whatAction==='add'){
                ...
                setTimeout(async ()=>{
                    ...
                    await categoryService.update(id,categoryRequest).catch(e=>{
                        console.log(e)
                    })
                    await showData(currentPage);
                    await setLoading(false)
                    await value.showToastMessage("Cập Nhật Thể Loại Thành Công!")
                    await handleClose();
                },1000)
            }
        }
    ...
    const movePage=(page)=>{
        if(whatActionMovePage==='find'){
            categoryService.findByAll(nameFind,statusFind,page).then(res=>{
                setList(res.data)
            }).catch(e=>{
                console.log(e)
            })
    ...
    const handleMovePre=()=>{
        if(whatActionMovePage==='find'){
            if(currentPage===1){
                categoryService.findByAll(nameFind,statusFind,totalPage.length).then(res=>{
                    setList(res.data)
                }).catch(e=>{
                    console.log(e)
                })
                ...
                categoryService.findByAll(nameFind,statusFind,currentPage-1).then(res=>{
                    setList(res.data)
                }).catch(e=>{
                    console.log(e)
                })
    ...
    const handleMoveNext=()=>{
        if(whatActionMovePage==='find'){
            if(currentPage===totalPage.length){
                categoryService.findByAll(nameFind,statusFind,1).then(res=>{
                    setList(res.data)
                }).catch(e=>{
                    console.log(e)
                })
    ...
    const handleFind= async ()=>{
        ...
        await categoryService.findByAll(nameFind,status,1).then(res=>{
            setList(res.data)
        }).catch(e=>{
            console.log(e)
        })
        await categoryService.findByAllTotalPage(nameFind,statusFind).then(res=>{
            let myArray=[]
            for(let i=1;i<=res.data;i++){
                myArray.push(i);
            }
            ...
            <div className="">
                <div className="p-[20px]">
                    <div className="title mb-[20px]">
                        ...
                        <span className="text-[22px] font-bold ml-[10px]">Quản lý thể loại</span>
                    </div>
                            ...
                            <div className="py-[20px]">
                                <div className="grid grid-cols-2">
                                    <div className="text-center">
                                        <span className="text-[#444] text-[14px] font-[500]">Tên thể loại :</span>
                ...
                <div className="flex justify-center items-center h-full">
                    <div className="modal-container p-[20px] bg-[#fff] rounded-[5px]">
                        <div className="flex justify-between">
                            <div><span className="font-[600]">Thêm thể loại</span></div>
                            <div className="cursor-pointer" onClick={handleClose}><i className="fa-solid fa-xmark"></i></div>
                        </div>
                        <div className="text-[14px]">
                            <div className="flex items-center">
                                <p className="mb-[5px] mt-[10px]"><span className="text-primary-red">*</span> Tên thể loại</p>
